...
# Directory for uploads (fallback for local storage)
...
# Local files list function (fallback)
def get_local_files_with_routing():
    files = []
...
# API for files list with routing (try holo, fallback to local)
...
async def files_with_routing():
    ...
    return get_local_files_with_routing()

# Local upload function (fallback)
async def local_upload_file(file: UploadFile):
    filepath = os.path.join(UPLOAD_DIR, file.filename)
    with open(filepath, "wb") as f:
        content = await file.read()
        f.write(content)
    return {"message": f"File {file.filename} uploaded locally (holo fallback)"}

# API for upload (try holo, fallback to local)
@app.post("/api/upload")
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        pass  # Fall back to local
    # Fallback to local (rewind file for local upload)
    file.file.seek(0)
    return await local_upload_file(file)

# Local download function (fallback)
def local_download_file(filename: str):
    filepath = os.path.join(UPLOAD_DIR, filename)
    if not os.path.exists(filepath):
        raise HTTPException(status_code=404, detail="File not found locally")
    return FileResponse(filepath, filename=filename)

# API for download (try holo, fallback to local)
...
async def download_file(filename: str):
    ...
    try:
        ...
        pass  # Fall back to local
    return local_download_file(filename)
...
if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=8000)    return {
        "holo_storage_ip": "138.0.0.1",
        "holo_dns_enabled": True,
        "upload_directory": UPLOAD_DIR,
        "default_interface": "wlan0",
